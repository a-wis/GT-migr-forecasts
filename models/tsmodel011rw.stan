...
model {
  ...
  y[2:(NI+1)] ~ normal(phi0 + y[1:NI], sigma[1:NI]);
}
...
generated quantities{
  ...
  // for (i in 1:(NG-NI)) {
    // yf1[1] = phi0 + y[NI];
   l_sigmaf[1] = normal_rng(sig_a*(phi0 + y[NI+1]),0.2858);
    sigmaf[1] = exp(l_sigmaf[1])/(1+exp(l_sigmaf[1]));
  // }
  for (i in 1:NI) yf[i]=normal_rng(phi0 + y[i],sigma[i]); // 
  
  yf[NI+1]=normal_rng(phi0 + y[NI+1],sigmaf[1]); //NI+1 is the first year of forecast
  
  if (NG-NI>1) {
  for (i in (NI+1):(NG-1)){
    l_sigmaf[i-NI+1] = normal_rng(sig_a*(phi0 + yf[i]),0.2858);
    sigmaf[i-NI+1] = exp(l_sigmaf[i-NI+1])/(1+exp(l_sigmaf[i-NI+1]));
    yf[i+1]=normal_rng(phi0 + yf[i],sigmaf[i-NI+1]);
  }
  }
}
